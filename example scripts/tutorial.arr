...
task-list :: List<{Any; (Any -> Boolean)}> = [list:
  {'First, type in the string `"hello"` and press enter. \n\n(Note that Strings in Pyret are case-sensitive.)';
    _ == "hello"},
  {'Sometimes, you may encounter a prompt that describes an ill-defined task. In this case, you should click on the `impossible` button instead of writing code. \n\nFor example, try answering this prompt: \n\n> Enter a number that is both greater than 0 and less than 0.';
    _ == impossible},
  {[list: "You can use the **up arrow** on your keyboard to cycle through your previous entries, and the **down arrow** to go back. Using these shortcuts, you can easily retrieve text you've previously typed, just like copying and pasting.", 'Try using the up arrow to submit `"hello"` again.'];
    _ == "hello"},
  {[list: "You can also define variables in the entry area. Anything you define and bind to a variable can be accessed in later submissions.", "(Note that when you submit an entry that contains only a variable binding, the prompt will be reprinted.)", "We've defined a function `foo`. Try defining the value `a = 4` and calling `foo` on `a`."];
      _ == "Correct"},
...
closing-prompt = [list:"Nice work! You have successfully completed the Data Druid tutorial.", "If you ever find that your code is taking a long time to execute, or you've accidentally written an infinite loop, you can use the **Stop** button in the upper right-hand corner to terminate currently running code. This button will not affect your current progress in the assignment.","At any point, you can use the **Restart** button to reload the program and start from the beginning. Note that this action will reset any progress you have made up to that point."]
